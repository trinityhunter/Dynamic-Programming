class Solution {
    ...
    public int longestPalindromeSubseq(String s) {
        ...
        // dp = new Integer[s.length()+1][sb.toString().length()+1];

        // int ans = helper(s, sb.toString(), s.length(), sb.toString().length());
        ...
        int[] prev = new int[sb.toString().length()+1];

        for(int i=0; i<s.length()+1; i++){
            int[] curr = new int[sb.toString().length()+1];
